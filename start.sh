...
# Check deps
command -v pnpm >/dev/null 2>&1 || { echo "[ERROR] pnpm required. Run: npm install -g pnpm"; exit 1; }
...
if [ -z "$DATABASE_URL" ] || [[ "$DATABASE_URL" != mongodb* ]]; then
  echo "[ERROR] Set DATABASE_URL in .env to your MongoDB Atlas connection string"
